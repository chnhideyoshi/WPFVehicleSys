...
namespace WpfCalendar.Model
{
    public static class ChinaDate
    {
        ...
        static ChinaDate()
        {
            ...
            nHoliday.Add("0115", "元宵");
            nHoliday.Add("0505", "端午");
            nHoliday.Add("0707", "七夕");
            nHoliday.Add("0815", "中秋");
            nHoliday.Add("0909", "重阳");
            nHoliday.Add("1208", "腊八");
        }
        #endregion
        /// <summary>
        /// 获取农历
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public static string GetChinaDate(DateTime dt,ref MyDate mydate)
        {
            if (dt > china.MaxSupportedDateTime || dt < china.MinSupportedDateTime)
            {
                //日期范围：1901 年 2 月 19 日 - 2101 年 1 月 28 日
                mydate.Year = "农历查询范围是";
                mydate.Month = "1901年2月19日";
                mydate.Day = "";
                mydate.SolorTerm = "到";
                mydate.Holiday = "2101年1月28日";
                mydate.ChinaHoliday = "请返回重新查询";
                return string.Empty;
            ...
            mydate.Year = GetYear(dt);
            mydate.Month = GetMonth(dt);
            mydate.Day = GetDay(dt);
            string strJQ = GetSolarTerm(dt);
            if (strJQ != "")
            {
                str += " [" + strJQ + "]";
                mydate.SolorTerm = strJQ;
            }
            string strHoliday = GetHoliday(dt);
            if (strHoliday != "")
            {
                str += " " + strHoliday;
                mydate.Holiday = strHoliday;
            }
            string strChinaHoliday = GetChinaHoliday(dt);
            if (strChinaHoliday != "")
            {
                str += " " + strChinaHoliday;
                mydate.ChinaHoliday = strChinaHoliday;
            }
        ...
        public static string GetYear(DateTime dt)
        {
            ...
            string str = string.Format("{1}{2}[{0}年]", yearSX[yDZ], yearTG[yTG], yearDZ[yDZ]);
            return str;
        }
